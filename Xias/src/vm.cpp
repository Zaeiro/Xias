...
#define READ_CONSTANT(type) constants[(++ip)->Data].type;
...
namespace Xias {
	...
	void Vm::CallFunction(std::vector<OpType>& function, std::vector<Value>& constants)
	{
		Value* sp = m_Stack.data();
        OpType* ip = function.data();
        OpType* ie = &(*function.begin()) + function.size();
		if (ip == ie)
			return;
		...
		while (ip != ie)
		{
			...
			switch (ip->Op)
			{
                ...
                // Stack Usage
                case Instruction::push_value: { *sp = constants[(++ip)->Data]; sp++; break; }
                case Instruction::push_size:
                {
                    x_ulong size = READ_CONSTANT(UInt);
                    std::memcpy(sp, &constants[(++ip)->Data], size * sizeof(Value));
                    sp += size;
                    break;
                }
